...
public class Player extends SpriteBatch implements InputProcessor,ICameraEventListener{
	...
	private void updateHandCardsPosition(){
		...
		float OriginY;
		for (;i<imax;i++){
			OriginY = handCards.get(i).getOriginY();
			handCards.get(i).setOrigin(handCards.get(i).getOriginX(), 0);
			if (handCards.get(i)==handCardClick){
				...
				handCards.get(i).setIsFaceUp(true);
				System.out.println(handCards.get(i).getOriginX() +"----"+handCards.get(i).getOriginY());
			}
			if (y>=maxY){
				deltaY = -deltaY;				
			}
			...
			y+=deltaY;
			//handCards.get(i).setOrigin(handCards.get(i).getOriginX(), OriginY);
		}		
	}
